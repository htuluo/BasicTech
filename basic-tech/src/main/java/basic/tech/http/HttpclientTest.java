...
//        }
     ...
     * 华为付费下载的测试
     */
    private static void testHuaWeiFFXZ() throws Exception {
        ...
        String response = HttpClientUtil.doPostWithUrlParam(url, param, headers);
        ...
        for (int i = 0; i < 5; i++) {
            new Thread(new Runnable() {
                ...
                public void run() {
                    ...
                    String url = "http://127.0.0.1:8080/test/post";
                    System.out.println("=====" + url);
                    String jsonString = "{'aaa':'bbb'}";
                    try {
                        System.out.println("++++" + HttpClientUtil.doPost(url, jsonString));
                    } catch (Exception e) {
                        e.printStackTrace();
                    }
//                    CloseableHttpResponse response = HttpClientUtil.doPostWithHeaderJsonString(url, jsonString,null,null);
//                    System.out.println("+++++"+HttpClientUtil.parseResponseToStr(null,response));
